...
def load_yearly_data():
    """원본 yelpzip.csv에서 전체 데이터 로드 (시간·식당·별점 분석용)"""
    p = BASE / "yelpzip.csv"
    if not p.exists():
        return None
    df = pd.read_csv(p, usecols=["date", "label", "rating", "prod_id"])
    df["date"]  = pd.to_datetime(df["date"])
    df["year"]  = df["date"].dt.year
    df["label"] = df["label"].map({-1: 1, 1: 0})   # -1→사기(1), 1→정상(0)
    return df
...
# ╚══════════════════════════════════════════════╝
    ...
    with tab_time:
        # 연도별 분석은 전체 원본 데이터(yelpzip.csv) 사용
        df_yr = df_yearly if df_yearly is not None else df.assign(
            label=df["label"])
        st.markdown('<div class="section-header">📅 연도별 사기 리뷰 발생 추이 (전체 2004~2015)</div>',
                    unsafe_allow_html=True)
        yearly   = df_yr.groupby(["year", "label"]).size().reset_index(name="count")
    ...
    with tab_prod:
        ...
        if df_yearly is not None and "prod_id" in df_yearly.columns:
            df_p = df_yearly.copy()
            prod_stats_full = df_p.groupby("prod_id").agg(
                total=("label","count"), fraud=("label","sum"),
                avg_rating=("rating","mean")).reset_index()
            prod_stats_full["fraud_rate"] = prod_stats_full["fraud"] / prod_stats_full["total"]
            ...
            with col_p2:
                ...
                for lbl, color, name in [(0, C_NORMAL, "정상"), (1, C_FRAUD, "사기")]:
                    ...
                    prod_r = sub.groupby("prod_id")["rating"].mean()
                    fig_p2.add_trace(go.Histogram(
                        x=prod_r, name=name, marker_color=color,
            ...
            st.warning("전체 데이터(yelpzip.csv)를 불러올 수 없습니다. prod_id 컬럼이 필요합니다.")

    with tab_rating:

        if df_yearly is not None:
            df_yr_r = df_yearly.copy()
            ...
            with col_r1:
                st.markdown('<div class="section-header">📊 별점별 리뷰 수 (절대값)</div>',
                            unsafe_allow_html=True)
                r_fraud  = df_yr_r[df_yr_r.label==1]["rating"].value_counts().sort_index()
                r_normal = df_yr_r[df_yr_r.label==0]["rating"].value_counts().sort_index()
                fig_ra = go.Figure()
            ...
            with col_r2:
                st.markdown('<div class="section-header">📊 별점별 비율 (%) — 양극단 패턴 확인</div>',
                            unsafe_allow_html=True)
                r_fraud_pct  = (df_yr_r[df_yr_r.label==1]["rating"]
                                .value_counts(normalize=True).sort_index() * 100)
                r_normal_pct = (df_yr_r[df_yr_r.label==0]["rating"]
                                .value_counts(normalize=True).sort_index() * 100)
                fig_rb = go.Figure()
            ...
            st.warning("yelpzip.csv 파일이 없어 별점 분포를 표시할 수 없습니다.")
